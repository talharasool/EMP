...
var oldPolyLines = [GMSPolyline]()
   ...
   func isHiddenNBIV(val : Bool){
    ...
    override func viewDidLoad() {
        ...
        self.setUpAction()
        self.googleMapView.settings.allowScrollGesturesDuringRotateOrZoom = false
...
//                                                  polyline.map = self.googleMapView
                  ...
                  //  let line = GMSPolyline(path: <#T##GMSPath?#>)
                    
                    //self.googleMapView.
//
                    
//                    var path = self.polyline.path
//                    path?.pathOffset(byLatitude: self.currentCoordinate.coordinate.latitude, longitude:  self.currentCoordinate.coordinate.longitude)
//
//                    self.polyline.path = path
//
//                    print("\n\nThe paths are here ",path)
//                    self.polyline.map = self.googleMapView
                 //self.draw(src: CLLocationCoordinate2D(latitude: lat, longitude: long), dst: CLLocationCoordinate2D(latitude: arr[0].lat!, longitude: arr[0].long!))
                    let dest = CLLocationCoordinate2D(latitude: self.compareArr[0].lat!, longitude: self.compareArr[0].long!)
                    print("The dest",dest)
                    print("The location\n")
                    print(self.currentCoordinateValue)
                    self.drawAgain(src: self.currentCoordinateValue, dst: dest)
                    for p in (0 ..< oldPolyLines.count) {
                       // oldPolyLines[p].map = nil
                    }
...
extension HomeVC : CLLocationManagerDelegate, GMSMapViewDelegate{
    ...
    func mapView(_ mapView: GMSMapView, didTapAt coordinate: CLLocationCoordinate2D) {
        ...
        if self.okBtnOutlet.alpha == 1{
            self.placeMarkerOnMapOnTap(coordinate: coordinate)
        }
    //    self.placeTempMarkerAtCoordinate(currentCoordinate: coordinate)
    ...
    func placeMarkerOnMapOnTap(coordinate : CLLocationCoordinate2D){
...
extension HomeVC : StoryboardInitializable{
    ...
    //Again
        ...
        func drawAgain(src: CLLocationCoordinate2D, dst: CLLocationCoordinate2D){
           ...
           // self.startAnimating()
            ...
            let url = URL(string: "https://maps.googleapis.com/maps/api/directions/json?origin=\(src.latitude),\(src.longitude)&destination=\(dst.latitude),\(dst.longitude)&sensor=false&mode=driving&key=\(Keys.AppKeys.googleMapKey)")!
                            ...
                            if let json : [String:Any] = try JSONSerialization.jsonObject(with: data!, options: .allowFragments) as? [String: Any] {
                               ...
                               // self.startLocation = address["start_address"] as! String
                                ...
                                DispatchQueue.main.async(execute: {
                                    ...
                                    self.oldPolyLines.append(self.polyline)
